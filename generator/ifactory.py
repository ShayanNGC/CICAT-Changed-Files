...
class COMPONENT_FACTORY(CI_FACTORY):
    ...
    def load (self, ctypeList ):
          ...
          for row in sheet.rows:
            # Code addition: if we encounter an empty row, break
            if row[0].value == None:
               break
...
class CTYPE_FACTORY(CI_FACTORY):
    ...
    def load (self):
          ...
          ret = []         
          for row in sheet.rows:
            # If the component row doesn't have an ID, we can break (assume we got everything)
            # This should keep us from going too deep into the Ctype list
            if row[0].value == None:
               break
...
class SYSTEM_FACTORY(CI_FACTORY):
    ...
    def load (self):
          ...
          ret = []         
          for row in sheet.rows:
            # if we encounter an empty row, break
            if row[0].value == None:
               break
