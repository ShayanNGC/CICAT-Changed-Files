...
def GenTTPSequence (dataset, factory, patSeq, aName, actFlag, trace):
    ...
    for p in patSeq:
        ...
        cmpName = p[0] #component name
        tacpat  = p[1] #tactic pattern
        ...
        for t in tacpat:
           ttp = TTP_FILTER (dataset, factory, cmpName, t, platflag, actor, actFlag, trace)  
           # check if the ttp is empty before appending it
           if ttp != []:
                foo.append (TTP_FILTER (dataset, factory, cmpName, t, platflag, actor, actFlag, trace) )
        # check if foo is empty before appending it
        if foo != []:
            ret.append (foo)
